...
import { CustomerFormMode, StatusType, type Customer } from '@/types'
...
} from '@/old-components/form'
import { CustomerService } from '@/services/customerService'

const toast = useToast()
// props
const { mode, customerName = '' } = defineProps<{
...
const isUpdateSuccess = defineModel<boolean>('isUpdateSuccess', {
  default: false,
})
const isCreateSuccess = defineModel<boolean>('isCreateSuccess', {
  default: false,
})
...
const dialogHeader = computed(() => {
  switch (mode) {
    ...
    case CustomerFormMode.View:
      return customerName
        ? `Detail information of ${customerName}`
...
// variables
const statusOptions: { name: string; type: StatusType }[] = Object.values(
  StatusType
).map((status) => ({
  name: status,
  type: status,
...
const updateCustomer = async () => {
  try {
    const result = await CustomerService.patchCustomer(
      customer.value.id,
      customer.value
    )
...
const resolver = (e: FormResolverOptions): Record<string, any> => {
  ...
  if (
    !customer.value.engagedParty['@referredType'] &&
    !values.engagedPartyType
  ) {
...
<template>
  <Dialog v-model:visible="visible" :header="dialogHeader">
    <Form v-slot="$form" @submit="handleSubmit" :resolver>
      <!-- Form content -->
      <div class="flex flex-col gap-y-6 w-[35rem]">
        ...
        <div class="flex gap-4">
          <div class="flex flex-col flex-1 gap-1">
            ...
            <Message
              v-if="$form.customerName?.invalid"
              size="small"
              severity="error"
              variant="simple"
              >{{ $form.customerName.error?.message }}</Message
            >
          </div>
          <div
            v-if="mode !== CustomerFormMode.Create"
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex gap-4">
            <div class="flex flex-col flex-1 gap-1">
              <label class="text-sm font-medium" for="startDate"
                >Start date</label
              >
              <DatePicker
                v-if="!readonly"
              ...
              <p v-else class="text-sm">
                {{ customer.validFor.startDateTime.toLocaleDateString() }}
              </p>
            </div>
            <div class="flex flex-col flex-1 gap-1">
              ...
              />
              <p v-else class="text-sm">
                {{ customer.validFor.endDateTime.toLocaleDateString() }}
              </p>
            </div>
          </div>
        </div>
